...
<div class="table-responsive">
    ...
    <div class="col-md-12" style="margin-bottom: 10px">
        ...
        <div class="col-md-5" style="margin-bottom: 0px">
            @include('elements.search.selectCat', array('text' => $lang->get(478), 'id' => "cat_search", 'onchange' => "onCatSearchSelect(this)"))  {{--Parent Category--}}
        </div>
    ...
    <table id="data_table" class="table table-bordered table-striped table-hover">
        ...
        <tr>
            ...
            <th>{{$lang->get(478)}}</th> {{--Parent Category--}}
            <th>{{$lang->get(539)}}</th> {{--Items count--}}
...
<script>
    ...
    function buildOneItem(item){
        ...
        return `
            <tr>
                ...
                <button type="button" class="q-btn-all q-color-alert waves-effect" onclick="showDeleteMessage('${item.id}', '{{ url("categorydelete") }}')">
    ...
    function initTableHeader(){
        var html = `
            ...
            <th>{{$lang->get(70)}} </th>                                                                                                   {{--Image--}}
            <th>{{$lang->get(478)}} <img <img onclick="tableHeaderSort('parent');" src="${utilGetImg('parent')}" class="img-fluid" style="margin-left: 10px; width: 20px; float: right;"></th> {{--Parent Category--}}
            <th>{{$lang->get(539)}} </th>                                                                                                   {{--Items count--}}
