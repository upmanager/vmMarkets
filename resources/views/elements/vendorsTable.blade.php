...
<div class="table-responsive">
    ...
    <table id="data_table" class="table table-bordered table-striped table-hover">
        ...
        <tr>
            ...
            <th>{{$lang->get(580)}}</th> {{--Admin Commission--}}
...
<script>
    ...
    function buildOneItem(item){
        ...
        return `
            <tr>
                <td>${item.id}</td>
                <td>${item.name} <br>(${item.shopName})</td>
                <td>
                ...
                <td>
                    ...
                    <button type="button" class="q-btn-all q-color-alert waves-effect" onclick="showDeleteMessage('${item.id}', '{{ url("vendorDelete") }}')">
    ...
    function initTableHeader(){
        var html = `
            ...
            <th>{{$lang->get(580)}} <img <img onclick="tableHeaderSort('commission');" src="${utilGetImg('commission')}" class="img-fluid" style="margin-left: 10px; width: 20px; float: right;"></th> {{--Admin Commission--}}
